...
- `/analyze` - Базовый анализ криптовалюты (только котировки)
- `/enhanced` - **Расширенный анализ** (котировки + новости + PDF-отчет)
...
- `/refresh_news <SYMBOL>` - (Админ) Принудительно обновить новости для символа
...
**⚠️ Важно:** Анализ выполняется на **дневном таймфрейме** (1day) для максимальной точности прогнозов.

1. **Технический анализ (дневные данные):**
   - Тренд (MA7 vs MA30)
...
3. **Расширенный анализ (`/enhanced`):**
   - **Автопоиск свежих новостей** при каждом запросе
   - **Кэширование новостей** для повторных запросов (обновление каждые 24ч)
   - **Мультиуровневый анализ:** технические данные + новостной фон + тональность
   - **Ключевые темы** и анализ настроений рынка
